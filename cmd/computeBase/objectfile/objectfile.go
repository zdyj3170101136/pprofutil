...
func Open(filePath string, m *profile.Mapping) (*ObjectFile, error) {
	...
	if elfMagic := string(header[:]); elfMagic == elf.ELFMAG {
		f, err := open(filePath, m.Start, m.Limit, m.Offset, "", m.BuildID)
		if err != nil {
...
func open(filePath string, start, limit, offset uint64, relocationSymbol string, buildID string) (*ObjectFile, error) {
